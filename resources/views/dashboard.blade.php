        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="card p-4 border-0 shadow-sm text-white"
                    style="background: linear-gradient(135deg, #4e73df 0%, #224abe 100%); border-radius: 20px;">
            ...
            <div class="row g-4">
                ...
                <div class="col-12">
                    <div class="card border-0 shadow-sm rounded-4">
                        <div class="card-body p-4">
                            <h5 class="fw-bold mb-4 text-primary"><i class="bi bi-person-plus-fill me-2"></i>Registrasi
                                Guru Wali</h5>
                            <form action="{{ route('admin.storeGuru') }}" method="POST" class="row g-3">
                                @csrf
                                <div class="col-md-3">
                                    <input type="text" name="name"
                                        class="form-control bg-light border-0 p-3 rounded-3" placeholder="Nama Guru"
                                        required>
                                </div>
                                <div class="col-md-3">
                                    <input type="email" name="email"
                                        class="form-control bg-light border-0 p-3 rounded-3" placeholder="Email"
                                        required>
                                </div>
                                <div class="col-md-3">
                                    <input type="password" name="password"
                                        class="form-control bg-light border-0 p-3 rounded-3" placeholder="Password"
                                        required>
                                </div>
                                <div class="col-md-3">
                                    <button type="submit" class="btn btn-primary w-100 py-3 fw-bold rounded-3">Simpan
                                        Guru</button>
                                </div>
                            </form>
                        </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                        <div class="card-header bg-white border-0 p-4">
                            <h5 class="fw-bold mb-0 text-dark"><i
                                    class="bi bi-person-lines-fill me-2 text-primary"></i>Daftar Guru & Aktivitas</h5>
                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge bg-info bg-opacity-10 text-info rounded-pill px-3">
                                                    {{ \App\Models\Message::where('sender_id', $guru->id)->orWhere('receiver_id', $guru->id)->count() }}
                                                    Pesan
                                                </span>
                                            </td>
                                            <td class="text-end pe-4">
                                                <a href="{{ route('dashboard', ['guru_id' => $guru->id]) }}"
                                                    class="btn btn-sm btn-outline-primary rounded-pill me-1">
                                                    <i class="bi bi-eye"></i> Intip
                                                </a>
                                                <form action="{{ route('admin.destroyGuru', $guru->id) }}"
                ...
                <div class="col-12">
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                                ...
                                <div class="col-md-8">
                                    <form action="{{ route('dashboard') }}" method="GET"
                                        ...
                                        <input type="text" name="search"
                                            ...
                                            placeholder="Cari nama..." style="width: 180px;">
                                        <button type="submit"
                                            class="btn btn-sm btn-dark rounded-pill px-4">Filter</button>
                            ...
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    @foreach ($allMessages as $msg)
                                        <tr>
                                            <td class="ps-4 small text-muted">{{ $msg->created_at->format('d/m H:i') }}
                                            </td>
                                            <td><span
                                                    class="badge bg-{{ $msg->sender->role == 'guru' ? 'primary' : 'success' }}">{{ $msg->sender->name }}</span>
                                            </td>
                                            <td class="fw-bold">{{ $msg->receiver->name }}</td>
                                            <td>
                                                <div class="p-2 bg-light rounded-3 small border">{{ $msg->body }}
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        <div class="p-4">
                            {{ $allMessages->appends(request()->query())->links('pagination::bootstrap-5') }}</div>
                    </div>
                </div>
            </div>
                    ...
                    @foreach ($messages as $msg)
                        <div class="card border-0 shadow-sm rounded-4 mb-3 p-4 border-start border-primary border-5">
                            ...
                            <form action="{{ route('chat.send') }}" method="POST" class="d-flex gap-2">
                                ...
                                <input type="text" name="body" class="form-control bg-light border-0 rounded-4"
                ...
                <div class="col-md-5">
                    <div class="card border-0 shadow-sm rounded-4 p-4">
                        ...
                        <form action="{{ route('chat.send') }}" method="POST">
                            ...
                            <textarea name="body" class="form-control border-0 bg-light p-3 mb-3" rows="4" placeholder="Tulis pesan..."
                                required></textarea>
                            <button type="submit" class="btn btn-primary w-100 py-3 fw-bold rounded-3 shadow">Kirim
                                Pesan</button>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 p-4">
                        ...
                        <div class="overflow-auto" style="max-height: 450px;">
                            @foreach ($myMessages as $msg)
                                <div
                                    class="mb-3 d-flex flex-column {{ $msg->sender_id == auth()->id() ? 'align-items-end' : 'align-items-start' }}">
                                    <div class="p-3 rounded-4 {{ $msg->sender_id == auth()->id() ? 'bg-primary text-white' : 'bg-light text-dark' }}"
                                        style="max-width: 80%;">
                                        <div class="d-flex justify-content-between gap-3 mb-1">
                                            <span class="fw-bold"
                                                style="font-size: 10px;">{{ $msg->sender_id == auth()->id() ? 'SAYA' : $msg->sender->name }}</span>
                                            <span
                                                style="font-size: 9px opacity: 0.7;">{{ $msg->created_at->format('H:i') }}</span>
                                        </div>
